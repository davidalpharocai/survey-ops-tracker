...
export default async function AdminAuditPage({ searchParams }: PageProps) {
  ...
  return (
    <>
      <Link className="back" href="/admin">← Admin</Link>
      <h1>Audit log</h1>
      ...
      <form method="get" className="filterbar">
        ...
        <Link className="back" href="/admin/audit">Reset</Link>
      </form>
      ...
      {athena && !error && (
        <>
          ...
          {queryId && nextToken && (
            <p>
              <Link className="btn btn-sm" href={`/admin/audit?${nextParams.toString()}`}>
                Load next page →
              </Link>
            </p>
          )}
