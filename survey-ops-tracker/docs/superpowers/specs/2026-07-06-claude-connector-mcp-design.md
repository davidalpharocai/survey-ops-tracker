...
- **Allow is a POST** (server action) that re-verifies the Supabase session, `isAllowedEmail`, and `profiles.role='analyst'` **server-side at code-issuance time**, with an anti-CSRF token bound to the session. Codes are never issued from a GET. The page sets `Content-Security-Policy: frame-ancestors 'none'` (no security headers exist app-wide today — this page sets its own).
...
- **Refresh rotation with a ~60s grace window**: rotation is an atomic single-row update; within 60s of rotation the previous refresh token returns the **same newly-issued pair** (idempotent retry — absorbs lost responses on flaky networks so users aren't forced to re-login); reuse **after** the window is treated as theft and revokes all live tokens for that (user_id, client_id). Columns `rotated_at`/`replaced_by` support this.
