-- 068: add the Occam flag — a boolean project flag (like row_level_data /
-- terminations) surfaced in the project page's Flags section. Occam is one of
-- our internal tools; the flag marks projects that use it.
alter table public.survey_projects add column if not exists occam boolean not null default false;

-- Keep the field-change audit trail consistent — log Occam toggles like every
-- other flag. Verbatim rebuild of 028's audit_survey_project() with one added
-- audit_field line for `occam`; the existing survey_projects_audit trigger keeps
-- pointing at it.
