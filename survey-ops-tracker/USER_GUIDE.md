...
- The **More → Insights** page (top nav) — every tile and bar is clickable and opens the matching, pre-filtered List (overdue, a stage, a captain's projects, the unassigned pile). It rolls up the whole pipeline: active/scoping/closed counts, overdue and due-this-week, on-time delivery %, average cycle time, stage distribution, per-captain workload, budget vs spend, and top clients — all derived live from your projects.
