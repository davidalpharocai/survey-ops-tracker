...
export function useMergeProjects() {
  ...
  return useMutation({
    mutationFn: async ({ survivorId, loserId, survivorUpdate }: MergeArgs) => {
      // RPC first: it runs all guards (self-merge, existence, segmented) and the
      // atomic re-point + soft-delete. Applying the survivor's field picks only
      // after it succeeds means a rejected merge never mutates the survivor.
      const { error: mergeError } = await supabase.rpc('merge_projects', { p_survivor: survivorId, p_loser: loserId })
...
export function useMergeClients() {
  ...
  return useMutation({
    mutationFn: async ({ survivorId, loserId, survivorUpdate }: MergeArgs) => {
      const { error: mergeError } = await supabase.rpc('merge_clients', { p_survivor: survivorId, p_loser: loserId })
      if (mergeError) throw mergeError
      if (Object.keys(survivorUpdate).length > 0) {
