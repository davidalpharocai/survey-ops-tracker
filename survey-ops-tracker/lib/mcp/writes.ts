...
export const PROJECT_WRITE_FIELDS = [
  ...
  // Added 2026-09-18 (migration 117): which route delivered the respondents, on
  // a survey fielded both by blast and by PureSpectrum. mcp_write_project must
  // name these too — a patch key its body does not mention is dropped SILENTLY
  // and update_project still reports ok.
...
export const UNDOABLE_FIELDS = new Set<string>([
  ...
  // 117: plain integers and a short enum, no side effects — they round-trip
  // through mcp_write_project's ::int coercion the same way n_actual does.
  'n_actual_panel', 'n_actual_blast', 'n_actual_split_method',
])
...
export interface ProjectCostRow {
  ...
  /** 117. Optional so this type still describes a row read before the migration. */
  route?: string | null
  created_at: string
}
...
export async function runLogCost(opts: {
  ...
  /** 117: which fielding route the money bought. */
  route?: 'blast' | 'panel' | null
  ...
  const { data, error } = await supabase.rpc('mcp_log_cost', {
    ...
    p_idem: opts.idemKey,
    p_actor: opts.actor,
    // Cast: types.ts is regenerated by hand and lags a migration applied
    // manually, so the generated Args type does not know p_route yet.
    p_route: opts.route ?? null,
  } as Database['public']['Functions']['mcp_log_cost']['Args'] & { p_route: string | null })
  if (error) throw new Error(error.message)
