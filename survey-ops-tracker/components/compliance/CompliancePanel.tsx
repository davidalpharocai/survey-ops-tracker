  ...
  return (
    <div className="flex flex-col gap-2 mb-4">
      <div className="flex items-center justify-between text-xs bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">
        {remaining > 0 ? (
          <span className="text-amber-600 dark:text-amber-400">Sending to compliance in {display}</span>
        ) : (
          <span className="text-amber-600 dark:text-amber-400">Sending to compliance…</span>
        )}
        {remaining > 0 && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={recalling || dispatching}
              className="text-xs border border-red-500/40 text-red-600 dark:text-red-400 hover:bg-red-500/10 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
      ...
      {dispatchError && (
        <div className="flex items-center gap-2">
          <p className="text-xs text-red-600 dark:text-red-400 flex-1">{dispatchError}</p>
          <button
            onClick={() => { dispatchFiredRef.current = false; dispatch() }}
            className="text-xs border border-amber-500/40 text-amber-600 dark:text-amber-400 hover:bg-amber-500/10 px-2 py-1 rounded transition-colors"
          >
            Send now
          </button>
        </div>
      ...
      {recallError && (
        <p className="text-xs text-red-600 dark:text-red-400 bg-red-400/10 rounded-lg px-3 py-2">{recallError}</p>
      )}
    </div>
  )
}
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          {submissions.map(s => {
            ...
            const noteStyle = s.status === 'rejected'
              ? 'text-red-600/80 dark:text-red-400/80 bg-red-400/10'
            ...
            return (
              <div key={s.id} className="rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between text-xs bg-muted/50 hover:bg-muted/80 transition-colors px-3 py-2 cursor-pointer"
                >
                ...
                {isExpanded && (
                  <div className="bg-muted/30 border-t border-border px-3 py-2.5 flex flex-col gap-1 text-xs">
                    <p className="text-foreground">
                      <span className="text-muted-foreground">Submitted:</span>{' '}
                      {formatDate(s.submitted_at)}{s.submitter_name ? ` by ${s.submitter_name}` : ''}
                    </p>
                    <p className="text-foreground">
                      <span className="text-muted-foreground">Sent to compliance:</span>{' '}
                    ...
                    {s.analyst_message && (
                      <p className="text-foreground bg-muted/50 mt-1 rounded px-2 py-1.5">
